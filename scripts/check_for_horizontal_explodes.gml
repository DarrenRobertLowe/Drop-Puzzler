...
{
    for (var column=(gridWidth-3); column>-1; column--) //-3 because we're checking the 2 cols to the right of each col
    {
        //show_debug_message("checking horizontal explodes on row:"+string(row) +" col:"+string(column));
        var gridItem = 0;
        ...
        or (gridItem.object_index == obj_block_yellow)
        {
            ...
            var item      = gridItem;
            var prevItem  = ds_grid_get(global.grid, column+1, row);
            var prevItem2 = ds_grid_get(global.grid, column+2, row);
